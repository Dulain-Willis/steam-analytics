...
- **staging** — one model per landing table (`stg_steam__<entity>`). Casts,
  renames, drops CDC bookkeeping columns. No joins, no business logic.
  Materialized as `view`. Schema: `stg`.
...
ADRs live in `docs/adr/` (created lazily as decisions get resolved during the
build — none yet). Open decisions tracked in issue #9 under "Open decisions to
resolve in-flight".
